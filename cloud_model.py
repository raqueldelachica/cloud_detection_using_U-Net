...
class CloudModel(pl.LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        if self.train_dataset.data is None:
            raise ValueError(
                print("x_train and y_train must be specified when CloudModel is instantiated to run training")
            )
    ...
    def validation_step(self, batch: dict, batch_idx: int):
        ...
        if self.val_dataset.data is None:
            raise ValueError(
                print("x_val and y_val must be specified when CloudModel is instantiated to run validation")
            )
    ...
    def _prepare_model(self):
        ...
        unet_model = smp.Unet(
            encoder_name=self.backbone, # model.encoder - pretrained backbone to extract features of different spatial resolution
            encoder_weights=self.weights, ## takes the weight of the images themselves
            in_channels=3, ## because we have 4 bands in each input image (RBG+I)
            classes=2,  # number of classes in the output channel, input is 4, output is 2
        )
        if self.gpu:
            unet_model.cuda()
